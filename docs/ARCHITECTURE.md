...
- `skills/`：核心运行资产。按阶段拆分 orchestrator 与子 skill，并在共享目录中维护 intake 模板、调研 rubric、创意评分卡和方案模板。这里承载实际工作流、阶段约束、评分规则和输出模板；对运行时必需的最小 contract，允许镜像进对应 `SKILL.md` 以适配单-skill 临时目录。
...
- `skills/shared/`：共享约束文件，沉淀输入字段、研究判定、创意评分与 proposal 结构，作为仓库维护时的 canonical contract。
